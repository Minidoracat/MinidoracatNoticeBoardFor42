...
- **Markdown 公告**：公告內容以 Markdown 撰寫，遊戲內自動排版顯示。**引擎限制與取捨**：遊戲的文字面板只有換色與換字級兩種樣式、沒有粗體／斜體字型；要補真正的粗體得替換遊戲其他畫面在用的字型，或自帶粗體字型並自行排版——而且公告支援全部語系，字型得像遊戲本身一樣按語系各準備一套（中／日／韓／泰／西里爾……每種字級各一份，光中文就數十 MB），漏掉的語系會直接缺字。我們判斷不值得，所以粗體＝琥珀色、斜體＝綠色、行內程式碼＝粉色，刪除線只去記號、不支援表格（完整差異見 `docs/ADMIN_GUIDE.md`）
